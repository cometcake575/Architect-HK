...
public class PlayerAnimPlayer : MonoBehaviour, IAnimPlayer
{
    ...
    private IEnumerator DoPlay()
    {
        ...
        var clip = hero.animCtrl.animator.GetClipByName(clipName);
        if (clip == null) yield break;
        
        _animTimeRemaining = overrideAnimTime ? animTime : clip.Duration;
        hero.animCtrl.PlayClip(clipName);
    ...
    private void Update()
    {
        ...
        _animTimeRemaining -= Time.deltaTime;
        if (clearXVel) HeroController.instance.rb2d.SetVelocityX(0);
        if (clearYVel) HeroController.instance.rb2d.SetVelocityY(0);
        if (_animTimeRemaining <= 0) Stop();
    }
