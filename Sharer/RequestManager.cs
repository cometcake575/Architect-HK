...
public static class RequestManager
{
    ...
    public const string LEVEL_TYPE = "hk_modern";
        ...
        Action<bool, List<LevelInfo>, int> callback)
    {
        ...
        ArchitectPlugin.Instance.Log($"Prepared fields {Time.time - current}");
        current = Time.time;
        
        using var request = UnityWebRequest.Post(URL + "/search-new", form);

        var operation = request.SendWebRequest();
        ArchitectPlugin.Instance.Log($"Sending request {Time.time - current}");
        current = Time.time;
        yield return operation;
        ArchitectPlugin.Instance.Log($"Received response {Time.time - current}");
        current = Time.time;
        ...
        ArchitectPlugin.Instance.Log($"Confirmed response {Time.time - current}");
        current = Time.time;
        
        var meta = response[0];
        response.RemoveAt(0);
        var levelInfo = LoadLevels(response);
        ArchitectPlugin.Instance.Log($"Finished parsing response {Time.time - current}");
    ...
    public static IEnumerator DownloadSave(string levelId, Text status)
    {
        ...
        UIManager.instance.slotFour.Prepare(GameManager.instance, true);
