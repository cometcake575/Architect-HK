...
public static class EditManager
{
    ...
    public static void Init()
    {
        ...
        typeof(HeroController).Hook("CanTakeDamage", BlockActionOrInvul);
        ...
        typeof(HeroController).Hook("CanDash", BlockAction);
        ...
        typeof(PersistentBoolItem).Hook("Awake", (Action<PersistentBoolItem> orig, PersistentBoolItem self) =>
        {
            ...
            {
                self.OnGetSaveState += (ref value) => value = false;
                self.enabled = false;
            }
            orig(self);
        });
    ...
    private static void OnSceneLoad(Action<GameManager, bool> orig, GameManager self, bool additiveGateSearch)
    {
