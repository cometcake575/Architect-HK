    ...
    ConfigValue[] config)
{
    ...
    public GameObject SpawnObject(Vector3 pos = default, string extraId = null, float extraRot = 0, float extraScale = 1, bool extraFlip = false)
    {
        if (!type.Prefab)
        {
            ArchitectPlugin.Instance.LogError($"Error - Prefab of {type.GetName()} is missing, cannot spawn");
            return null;
        }
        ...
        var toScale = _scale * extraScale;
        if (type.ScaleAction != null) type.ScaleAction.Invoke(obj, toScale);
        else if (!Mathf.Approximately(toScale, 1))
        {
            if (obj.GetComponent<HealthManager>())
            {
                var sp = new GameObject("[Architect] Scale Parent").transform;
                sp.position = obj.transform.position;
                obj.transform.SetParent(sp, true);
                sp.localScale *= toScale;
            } else obj.transform.localScale *= toScale;
        }
