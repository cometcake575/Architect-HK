...
public static class ScriptManager
{
    ...
    public static IEnumerable<(Func<ScriptBlock>, string)> CurrentBlocks => CurrentCategory.Blocks.Where(c => 
            c.Item2.IndexOf(Filter, StringComparison.OrdinalIgnoreCase) >= 0);
        ...
        Connection.LinkType linkType)
    {
        ...
        if (!Links.TryGetValue((sourceBlock, sourceEvent, block, trigger), out var value)) return;
        Links.Remove((sourceBlock, sourceEvent, block, trigger));
